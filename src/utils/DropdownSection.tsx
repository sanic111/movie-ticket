...
const DropdownSection = forwardRef<DropdownSectionHandle, DropdownSectionProps>(({
  ...
  return (
    <div className="dropdown-section">
      ...
      >
        {customHeader ? (
          customHeader // Nếu có customHeader thì render cái này
        ) : (
          <span className="title">{title}</span> // Nếu không thì render mặc định
        )}

        {!hideIcon && (
          <Icon
            name={isOpen ? "dropdownUp" : "dropdownDown"}
            className="icon"
          />
        )}
      </button>

      {(isOpen || !canToggle) && <div className="content">{children}</div>}
    </div>
  );
});
