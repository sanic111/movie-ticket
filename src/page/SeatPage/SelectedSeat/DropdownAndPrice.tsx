...
const DropdownAndPrice = forwardRef<DropdownAndPriceHandle, Props>(
  ({ selectedSeatsRef, seatRows, onContinue, onRemoveSeat, seatMapRef }, ref) => {
    ...
    const updateFromRef = useCallback(() => {
      ...
      setSeats((prevSeats) => {
        const prevSorted = prevSeats
          .slice()
          .sort((a, b) => a.seatId.localeCompare(b.seatId));
        const isDifferent =
          current.length !== prevSorted.length ||
          current.some((s, i) => s.seatId !== prevSorted[i]?.seatId);
    ...
    const handleContinue = () => {
      ...
      if (error) {
        ...
        // Highlight ghế cần chọn
        const highlightIds: string[] = [];

        for (const row of seatRows) {
          const selectedMap: Record<number, boolean> = {};
          row.forEach((s) => {
            selectedMap[parseInt(s.arrColIndex)] = seats.some(
          ...
          for (let i = 0; i < indices.length - 2; i++) {
            ...
            if (
              ...
              const seatToHighlight = row.find(
        ...
        {seats.length > 0 ? (
          ...
          <div className="choose-seat">
            <div className="empty-message">
              Quý khách vui lòng chọn vị trí ghế ngồi để tiếp tục
            </div>
